...
int main()
{
...

    {
        ...
        res = jio_memory_file_create(ctx, "csv_test_ugly.csv", &csv_file, 0, 0, 0);
        ASSERT(res == JIO_RESULT_SUCCESS);
    ...
    {
        jio_memory_file* csv_file;
        res = jio_memory_file_create(ctx, "csv_test_ugly2.csv", &csv_file, 0, 0, 0);
        ASSERT(res == JIO_RESULT_SUCCESS);
        jio_csv_data* csv_data;
        res = jio_parse_csv(ctx, csv_file, "( ͡° ͜ʖ ͡°)", true, true, &csv_data);
        ASSERT(res == JIO_RESULT_SUCCESS);
        uint32_t rows, cols;
        jio_csv_shape(csv_data, &rows, &cols);
        for (uint32_t i = 0; i < cols; ++i)
